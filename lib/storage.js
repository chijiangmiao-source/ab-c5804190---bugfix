...
export class LedgerStorage {
   ...
   *  4. 以链为准裁决操作索引：链上记录归位 committed，
   *     无对应记录的 pending 痕迹清除（见 _settleOperationIndex）
   */
  async _reconcile() {
    if (this.storage.getItem(K_QUARANTINE)) {
      try {
    ...
    if (!result.ok) {
      if (result.reason === 'anchor-behind' && anchor
        ...
        this._settleOperationIndex(chain);
        this._snapshot = this.readState();
        return;
      }
   ...
   * 操作索引裁决（持锁、链已从创世复算通过）：
   * 索引只是链的派生视图，一律以链为准重建——
   *  - 链上记录 -> committed 条目：崩溃于“链/锚点已写、索引未归位”窗口的
   *    提交在此归位，同内容重试按幂等返回原记录；
   *  - 无对应链上记录的 pending 条目 -> 删除：崩溃于记录形成之前，
   *    同一操作标识可立即重新提交，不会永久卡在“等待裁决”。
   * 能持锁即说明没有在途提交，残留的 pending 只会是崩溃痕迹。
   * 仅在索引实际变化时写回，避免空写广播 storage 事件。
   */
  _settleOperationIndex(chain) {
    const rebuilt = {};
    ...
    const raw = this.storage.getItem(K_OPERATION_INDEX);
    ...
    try {
      if (canonicalize(JSON.parse(raw)) === canonicalize(rebuilt)) return; // 已与链一致
    } catch {
      // 索引损坏：落到下面以链为准重建
    }
    this._guardedSet(K_OPERATION_INDEX, JSON.stringify(rebuilt));
