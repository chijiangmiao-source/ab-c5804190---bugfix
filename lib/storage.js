...
const K_OPERATION_INDEX = 'irrad.ledger.operations.v1';
const K_QUARANTINE = 'irrad.ledger.quarantine.v1';
const K_SUFFIX = 'irrad.ledger.suffix.v1';

export const STORAGE_KEYS = {
  K_CHAIN, K_ANCHOR, K_LOCK, K_INTENTS, K_OPERATION_INDEX, K_QUARANTINE, K_SUFFIX,
};
...
export class LedgerStorage {
  ...
  _handleStorageEvent(event) {
    const watched = [K_CHAIN, K_ANCHOR, K_QUARANTINE, K_SUFFIX, K_INTENTS, K_OPERATION_INDEX];
  ...
  async _submitInner(input) {
    ...
    return this._exclusive(async () => {
      ...
      const identity = operationIdentity(content);
      const operationIndex = readJson(this.storage, K_OPERATION_INDEX, {});
      const indexed = operationIndex[content.opId];
      if (indexed) {
        const indexedBlock = state.chain[indexed.seq - 1];
        if (indexed.phase === 'committed' && indexedBlock && sameContent(indexedBlock, content)) {
          this._snapshot = state;
          return { block: indexedBlock, reused: true };
        }
        if (indexed.identity === identity) {
          throw new LedgerError('OPID_PENDING', '该操作标识正在等待持久化裁决，请稍后重试', {
            opId: content.opId,
          });
        }
      ...
      operationIndex[content.opId] = {
        phase: 'pending',
        identity,
        seq,
      };
      this._guardedSet(K_OPERATION_INDEX, JSON.stringify(operationIndex));
   ...
   *  3. 清理/认领孤儿意图与操作索引 pending 残留（上次持锁者崩溃的唯一痕迹），
   *     使重开后只存在完整记录或完全无记录两种判定
   */
  async _reconcile() {
    if (this.storage.getItem(K_QUARANTINE)) {
      try {
    ...
    if (!result.ok) {
      if (result.reason === 'anchor-behind' && anchor
        ...
        this._reconcileOperationIndex(chain);
        this._snapshot = this.readState();
        return;
      }
  ...
  /**
   * 操作索引裁决（持锁、链已从创世复算通过）：
   * 索引里 phase=pending 的条目只可能是上次持锁者在两段提交窗口内崩溃的
   * 残留（正常提交在同一持锁窗口内完成 pending -> committed）。逐条对照
   * 链上实际，保证重开后只判定为完整记录或完全无记录：
   *  - 槽位 seq 上存在同 opId、同业务内容的记录 -> 记录已完整落库，
   *    补记为 committed（同内容重试由此幂等返回原记录）；
   *  - 否则 -> 记录从未落库，按“完全无记录”删除条目，
   *    同一操作标识可立即重新提交，不再被“正在等待裁决”卡住。
   * 索引 JSON 损坏时以复算通过的链为唯一事实源整体重建。
   */
  _reconcileOperationIndex(chain) {
    let index = null;
    try {
      const parsed = readJson(this.storage, K_OPERATION_INDEX, {});
      if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) index = parsed;
    } catch {
      index = null;
    }
    if (index === null) {
      // 索引损坏/畸形：链上记录全部按已提交重建索引。
      const rebuilt = {};
      for (const block of chain) {
        rebuilt[block.opId] = {
          phase: 'committed',
          identity: operationIdentity(block),
          seq: block.seq,
          digest: block.digest,
        };
      }
      this._guardedSet(K_OPERATION_INDEX, JSON.stringify(rebuilt));
      return;
    }
    let changed = false;
    for (const [opId, entry] of Object.entries(index)) {
      if (!entry || entry.phase !== 'pending') continue;
      const block = chain[entry.seq - 1];
      if (block && block.opId === opId && operationIdentity(block) === entry.identity) {
        index[opId] = {
          phase: 'committed',
    ...
    if (changed) {
      this._guardedSet(K_OPERATION_INDEX, JSON.stringify(index));
    }
  }
